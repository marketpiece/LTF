DROP FUNCTION  IF EXISTS  napdata.rp(numeric); 
CREATE OR REPLACE FUNCTION napdata.rp(numeric)
 RETURNS SETOF text
 LANGUAGE sql
AS $function$
  select sub.imor_row  from (	
    select    td.partidano,bui.seqnobuilding,h.seqnoobj, to_number(REGEXP_REPLACE(trim(pp.seqnots),'\D+','')),      
          ...
          to_char(max(dpp.part*h.objectarea)
             /*case when coalesce(pho.area,0) > 0 then pho.area
                  when coalesce(dpp.part,0) > 0 then dpp.part
             else coalesce(dpp.divident,1) / case when coalesce(dpp.divisor,0) = 0 then 1 else dpp.divisor end
             end*/
           ,'FM00000') ||','|| /*Площ, върху която е учредено вещно право на ползване (кв.м)*/   
   ...
   --  left join Land l on l.property_id = p.property_id
    ...
    where td.documenttype_id = 21  
      ...
      and (select case when max(n.extractdate) is null then to_date('01.01.1998','dd.mm.yyyy')
            ...
            where substr(n.filename,1,2) = 'RP' and n.municipality_id = td.municipality_id) <= coalesce(td.user_date,td.doc_date)
   ...
   union all
    ...
    select    td.partidano,999,99,to_number(REGEXP_REPLACE(trim(pp.seqnots),'\D+','')),        
         ...
         -- '"'||rpadc(max(pp.name),50)||','||  
          ...
          to_char(max(dpp.part*l.landarea)
             /*case when coalesce(pl.area,0) > 0 then pl.area
                  when coalesce(dpp.part,0) > 0 then dpp.part
             else coalesce(dpp.divident,1) / case when coalesce(dpp.divisor,0) = 0 then 1 else dpp.divisor end
             end*/
           ,'FM00000') ||','|| /*Площ, върху която е учредено вещно право на ползване (кв.м)*/
