DROP FUNCTION  IF EXISTS  napdata.po(numeric); 
CREATE OR REPLACE FUNCTION napdata.po(numeric)
 RETURNS SETOF text
 LANGUAGE sql
AS $function$
  Select 
        ...
        '"'||rpadc(pp.name,50) ||'",'|| /*Име на собственик*/  
        '"'||rpadc(case ts.kind_idn when '10' then '1'  
                               when '11' then ' '  
                               when '21' then '2' end ,1) ||'",'|| /*Индикатор за ДЗЛ (1-ЕГН, 2-ЮЛ, ' ' - чужденец)*/  
        '"'||rpadc(case when ts.kind_idn in ('10','21') then ts.idn end ,10) ||'",'|| /*ЕГН, БУЛСТАТ (Служебен номер на НАП) в зависимост от индикатора*/  
        ...
        '"'||rpadc(REGEXP_REPLACE(max(city.name),'[,""]',' ','g') ,25) ||'",'|| /*Адрес : Име населено място*/  
        ...
        '"'||rpadc(max(con.shortname),2) ||'",'|| /*Адрес : Държава*/  
        '"'||rpadc(a.nom,4) ||'",'|| /*Адрес : номер */  
        '"'||rpadc(a.entry,2) ||'",'|| /*Адрес : вход*/  
        '"'||rpadc(a.floor,2) ||'",'|| /*Адрес : етаж*/  
        '"'||rpadc(a.apartment,3) ||'",'|| /*Адрес : апартамент*/ 
   ...
   join Debtpartproperty dpp on dpp.debtsubject_id = ds.debtsubject_id 
                            ...
                            and dpp.typedeclar = to_char(pp.typedeclar)
  ...
  where td.documenttype_id = 21   
    ...
    and (select case when max(n.extractdate) is null then to_date('01.01.1998','dd.mm.yyyy')
          ...
          where substr(n.filename,1,2) = 'PO' and n.municipality_id = td.municipality_id) <= coalesce(td.user_date,td.doc_date)
  ...
  Group by
      ...
      pp.seqnots,  
      pp.name,
      case ts.kind_idn when '10' then '1'
                             when '11' then ' '
                             when '21' then '2' end ,
      case when ts.kind_idn in ('10','21') then ts.idn end,
      ...
      to_char(td.change_date,'yyyymmdd')
  Order by td.partidano, to_number(REGEXP_REPLACE(trim(pp.seqnots),'\D+','')), pp.name
$function$
;
